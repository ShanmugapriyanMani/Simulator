...
void DbusClient::speedoMeterRandSpeedGenerator()
{
    ...
    */
    QDBusReply<void> reply = dbusclient->call("setSpeedStartingAndEndingRange", speedoMeterNumber.first, speedoMeterNumber.second);
    if (!reply.isValid()) {
...
void DbusClient::rpmMeterRandRPMGenerator()
{
    ...
    for (int i = 0; i < rpmLimits.size() - 1; i++) {
        if (rpmGenerated > rpmLimits[i] && rpmGenerated <= rpmLimits[i + 1]) {
            rpmGeneratedInFloat += (rpmGenerated - rpmLimits[i]) * multipliers[i];

            for (int j = 0; j < i; j++) {
                rpmGeneratedInFloat += (rpmLimits[j + 1] - rpmLimits[j]) * multipliers[j];
            }
...
*/
    ...
    QDBusReply<void> reply = dbusclient->call("setRpmStartingAndEndingRange", rpmMeterNumber.first, rpmMeterNumber.second);
...
int DbusClient::engineTemperatureRange() const
{
    return m_engineTemperatureRange;
}

void DbusClient::setEngineTemperatureRange(int newEngineTemperatureRange)
{
    qDebug() << "Fuel range set to " << newEngineTemperatureRange;
    ...
    QDBusReply<void> reply = dbusclient->call("setEngineTemperatureRange", static_cast<int>(newEngineTemperatureRange));
    if (!reply.isValid()) {
        qDebug() << "Error calling setEngineTemperatureRange:" << reply.error().message(); }
    emit engineTemperatureRangeChanged();
}
